...
<head>
    ...
    <title>Create FAQ — OrangePlay Support</title>
    @fonts
...
<body>
    ...
    <div class="container">
        ...
        <div class="card">
            <form method="POST" action="/dashboard/faqs">
                @csrf
                <div class="form-group">
                    <label for="intent_code">Intent Code</label>
                    <input type="text" name="intent_code" id="intent_code" value="{{ old('intent_code') }}" maxlength="100" required>
                    <div class="form-hint">Unique identifier for this FAQ entry (e.g., "greeting", "pricing").</div>
                </div>

                <div class="form-group">
                    <label for="category">Category</label>
                    <input type="text" name="category" id="category" value="{{ old('category') }}" maxlength="100">
                    <div class="form-hint">Optional grouping label (e.g., "pricing", "payment").</div>
                </div>

                <div class="form-group">
                    <label for="keywords">Keywords</label>
                    <textarea name="keywords" id="keywords" rows="5" required>{{ old('keywords') }}</textarea>
                    <div class="form-hint">One keyword per line. Customer message matching any keyword will trigger this answer.</div>
                </div>

                <div class="form-group">
                    <label for="answer_text">Answer Text</label>
                    <textarea name="answer_text" id="answer_text" rows="6" required>{{ old('answer_text') }}</textarea>
                    <div class="form-hint">The reply sent to the customer when a keyword matches.</div>
                </div>

                <div class="form-group">
                    <label for="priority">Priority</label>
                    <input type="number" name="priority" id="priority" value="{{ old('priority', 50) }}" min="0" max="999">
                    <div class="form-hint">Higher = checked first. Greeting=100, Pricing=90, Payment=80.</div>
                </div>
